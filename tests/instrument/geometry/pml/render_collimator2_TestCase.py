...
class renderer_TestCase(TestCase):
    ...
    def test(self):
        ...
        cone_along_beam = operations.rotate(cone, transversal="1", angle="90.*deg")
        angles = np.arange(-160, 0., 15.)
        cones = [operations.rotate(cone_along_beam, vertical="1", angle='%s*deg' % a)
                    for a in angles]
        ...
        class Renderer(base):
            ...
            def end(self):
                return
        text = weave( all, open('collimator2.xml', 'wt'), print_docs = False, renderer=Renderer(), author='')
        return
